...
def get_portfolios(
    ...
    params=Depends(standard_params),
    db=Depends(get_db),
):
...
@portfolio_router.post(
    "/portfolios", status_code=status.HTTP_201_CREATED, response_model=PortfolioCreate
)
def create_portfolios(body: PortfolioCreate, db=Depends(get_db)):
    try:
        new_portfolio = Portfolio(
            user_id=body.user_id, name=body.name, description=body.description
        )
...
def delete_portfolio(portfolio_id: uuid.UUID, db=Depends(get_db)):
    try:
        ...
        if not portfolio:
            raise HTTPException(
                status_code=status.HTTP_204_NO_CONTENT, detail="Portfolio not found"
            )
